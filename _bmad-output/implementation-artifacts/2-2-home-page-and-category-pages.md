...
5. **Product card** — Each card shows: a 4:5 aspect image (16px radius, Warm Beige bg placeholder, `next/image`), product name (16px / 600 / Deep Muted Brown), price (16px / 400 / Warm Gray, formatted as USD). On **desktop hover**: an "Add to Cart" CTA fades in (300ms) and the image zooms 1.02×. On **mobile**: a persistent subtle CTA button is shown. **Out-of-stock** products (`stockQuantity === 0`) show a disabled "Out of Stock" label instead of the CTA. The card's image + name link to `/products/[id]`.

6. **Responsive grid & motion** — Grids use the existing Oren responsive grid (2-col mobile / 3-col tablet / 4-col desktop region of the 12-col system), ≥80px section gaps, 24px desktop / 20px mobile side padding. All hover/fade transitions use the calm `cubic-bezier(0.4,0,0.2,1)` easing over 300ms and respect `prefers-reduced-motion`.

7. **No regressions / no new remote assets** — The existing client-side auth flow, Redux store, and `Header`/`Footer` shell continue to work. No remote image hosts or font CDNs are introduced (images remain local SVGs under `/public/images/placeholders/`). `next.config.ts` is not modified (local SVGs already render via `dangerouslyAllowSVG`).
...
- [ ] **Task 2 — `ProductCard` component (AC: #5, #6)**
  ...
  - [ ] Layout per DESIGN.md → Product Card: Warm White `#fff8f4` container, 16px radius, ambient shadow (`.shadow-ambient`), **no border**; image 4:5 aspect, 16px radius, Warm Beige placeholder bg, via `next/image` with `fill` + `sizes`; name (`.text-headline-md` scaled to 16px or a 16px/600 class) Deep Muted Brown; price Warm Gray 16px/400 formatted `formatPrice(price)` → `$165.00` (add helper to `lib/helpers.ts` using `Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' })`).
  - [ ] Hover (desktop, `group` + `group-hover:`): CTA fades in (opacity 0→1, 300ms), image scales 1.02×. Mobile: persistent subtle CTA (always visible). Use Tailwind `md:` breakpoints + the existing easing.
...
- [ ] **Task 3 — `ProductGrid` with skeletons + infinite scroll (AC: #3, #4, #6)**
  - [ ] Implement `frontend/components/ui/ProductGrid.tsx` as a **Client Component** (`"use client"`). Props: `{ initialProducts: Product[]; total: number; queryParams: {...}; }`. Renders the responsive Oren grid of `ProductCard`s.
...
- [ ] **Task 4 — Home page rework (`/`) (AC: #1, #6, #7)**
  - [ ] Convert `frontend/app/page.tsx` into an **async Server Component** (remove the stale "Daily Food Store" demo content and the `BestSellerSection` mock-data section). Fetch `getProducts({ limit: 8 })` and `getCategories()` server-side in parallel (`Promise.all`).
...
- [ ] **Task 6 — Tests (AC: all)**
  - [ ] Add `ProductCard.test.tsx`: renders name + formatted price + image; shows "Add to Cart" affordance when in stock; shows disabled "Out of Stock" when `stockQuantity === 0`; links to `/products/:id`. Follow the style of `components/Button.spec.tsx` / `LoginForm.test.tsx` (Jest + React Testing Library).
...
- **Grid:** `.grid-oren` (12-col desktop, 3-col tablet, 2-col mobile) + `.container-oren` (64px desktop / 20px mobile side padding) + `.section-gap` (80px vertical). For a product grid you typically want a 4-col desktop region — either span the 12-col grid (`grid-column: span 3`) or use a dedicated `grid-cols-2 md:grid-cols-3 lg:grid-cols-4` Tailwind grid; both are acceptable, prefer the latter for a pure card grid and reserve `.grid-oren` for page-level layout.
...
(to be filled by dev agent — expected: types/product.ts, features/product/services/productApi.ts, components/ui/ProductCard.tsx, components/ui/ProductGrid.tsx, components/ui/ProductCardSkeleton.tsx, optional AddToCartButton.tsx, app/page.tsx (rework), app/categories/[name]/page.tsx, app/categories/[name]/loading.tsx, lib/helpers.ts (formatPrice), tests, plus removal of empty/stale stubs)
